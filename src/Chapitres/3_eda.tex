...
\textit{The Matthews correlation coefficient
...
\begin{figure}[H]
    ...
    \caption{Distribution of people born in the US or not.}
...
\textbf{Decision:} We opt for a \textbf{binary transformation} to capture the dominant signal:
\begin{itemize}
...
\textbf{Decision:} We will apply \textbf{domain-specific discretization} to capture these three distinct economic behaviors:
\begin{itemize}
...
\section{Correlation between different classes}

An interestig observation here is that male are more often married than female are. We have seen that married people tend to have an higher income. Since the model may learn that 'Married = High Income', the strong correlation between these variables leads it to infer that 'Male = High Income' by association. This is a great example of sociological bias embedded in the data, where the model may uses gender as a proxy for wealth.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Images/correlation_classes.png}
    \caption{An example of sociological biais. }
    \label{fig:heatmap}
\end{figure}
...
We decided not to use PCA because it removes interpretability. Our goal is to understand which factors influence income, and PCA replaces meaningful variables with abstract components. It also brings no performance gain, since our dataset is small and PCA does not separate the classes better than the original features. Also, PCA handles mixed numerical-categorical data poorly and leads to information loss.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Images/PCA.png}
    \caption{PCA applied to our dataset. }
    \label{fig:heatmap}
\end{figure}

It seems that PCA reveals a global separation trend, with high-income profiles clustering towards the extremes of the principal axes. However, the significant central overlap shows us that a simple linear boundary is insufficient for high accuracy. We therefore kept PCA only as a visualization, not as part of our final modeling pipeline. 
...
\begin{figure}[H]
    ...
    \caption{Distribution of numerical features' correlation}
    \label{fig:heatmap}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Images/pairplot.png}
    \caption{Pairplot Analysis of Numerical Features}
    \label{fig:pairplot}
\end{figure}
